...
class PDFContentExtractor:
    ...
    Features:
    - Text extraction using both PyMuPDF and pdfplumber
    - Image extraction with filtering of small/decorative images
    - Intelligent question parsing with options and answers
    - Structured JSON output matching the required format
    ...
    def extract_images_from_page(self, page_num: int) -> List[str]:
        ...
        Filters out very small images that are likely decorative
    ...
    def parse_questions_from_text(self, text: str, page_images: List[str], page_num: int) -> List[Dict]:
        """
        Parse questions from text with advanced pattern recognition

        Recognizes:
        - Question numbers (1., 2., etc.)
        - Multiple choice options [A], [B], [C], [D]
        - Answers (Ans [A], Ans. [B], etc.)
        - Question text spanning multiple lines
        """
        questions = []
    ...
    def _associate_images_with_questions(self, questions: List[Dict], page_images: List[str]):
        """
        Intelligently associate images with questions

        Uses simple distribution - in a more advanced version, this could use
        image position analysis or AI-based image-text matching
        """
        ...
        for i, question in enumerate(questions):
            ...
            if start_idx < len(page_images):
                # Assign first image as main question image
                question["images"] = page_images[start_idx]

                # Assign remaining images as option images
    ...
    def extract_all_content(self) -> Tuple[List[Dict], Dict]:
        ...
        Returns:
            Tuple of (questions_list, summary_dict)
        """
        all_questions = []
        extraction_summary = {
